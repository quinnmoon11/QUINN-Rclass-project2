...
knitr::read_chunk('QuinnProcessing.R')
```

Load needed packages. Instead of having R commands here, we can have R do it by pulls in the code from the R script according to the labels. Here the code chuck below is replaced with the `packages` code chunk from `QuinnProcessing.R`. 
...
Make histograms or densities of at mass by discrete category, to check for any potential category errors, extra categories, etc.  
...
Drop any variables (columns) that you wonʻt analyze, like comments.  If you have extra levels after dropping some values, you may want to relevel your factors (this may or may not happen). 
...
Save the clean data as RDS file and as a csv file. 
